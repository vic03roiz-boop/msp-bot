...
function buildGiveawayEmbed({ prize, endTimestamp, winnersCount, participantsCount, ended = false, winners = [], boosterBonus = false, conditions = null, host = null }) {
  const embed = new EmbedBuilder()
    .setColor(ended ? 0x95A5A6 : 0xC4D6C3)
  ...
  const footerText = host
    ? `Giveaway par ${host.name}${!ended ? ' • Clique sur le bouton pour participer !' : ''}`
    : (!ended ? 'Clique sur le bouton ci-dessous pour participer !' : null);

  if (footerText) {
    embed.setFooter({ text: footerText, iconURL: host?.avatarURL });
  }
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const embed = buildGiveawayEmbed({ prize, endTimestamp, winnersCount, participantsCount: 0, boosterBonus, conditions, host });
