...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // On accuse réception discrètement, puis on envoie le vrai message via le salon
    // directement (plus rapide que interaction.reply + fetchReply) pour pouvoir réagir
    // dessus sans délai perceptible pour les membres.
    await interaction.deferReply({ ephemeral: true });

    const message = await interaction.channel.send({ embeds: [embed] });
    await message.react(DROP_EMOJI);
    await interaction.editReply({ content: '✅ Drop publié !' });
