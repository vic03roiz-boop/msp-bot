...
function buildWheelSpinEmbed(wheel, participants, highlightIndex, final = false) {
  const lines = participants.map((id, i) =>
    i === highlightIndex ? `👉 <@${id}> 👈` : `<@${id}>`
  );
  return new EmbedBuilder()
    .setColor(final ? 0xF1C40F : 0xC4D6C3)
    .setTitle(final ? '🎉 Et le/la gagnant(e) est...' : '🎡 La roue tourne...')
...
async function handleWheelLaunch(interaction) {
  ...
  const delays = [120, 120, 140, 160, 180, 220, 260, 300, 360, 420, 500, 580, 660, 760, 860, 1000];
  const frames = delays.length;

  const frameIndex = (frame) => {
    const raw = winnerIndex - (frames - 1) + frame;
    return ((raw % participants.length) + participants.length) % participants.length;
  };

  await interaction.update({ embeds: [buildWheelSpinEmbed(wheel, participants, frameIndex(0))], components: [] });

  for (let frame = 1; frame < frames; frame++) {
    await sleep(delays[frame]);
    const isLast = frame === frames - 1;
    await interaction.editReply({
      embeds: [buildWheelSpinEmbed(wheel, participants, frameIndex(frame), isLast)],
    }).catch(() => {});
  }
