...
async function handleTicketOpen(interaction) {
  ...
  if (config.ticketStaffRoleId) {
    permissionOverwrites.push({ id: config.ticketStaffRoleId, allow: ['ViewChannel', 'SendMessages', 'ReadMessageHistory'] });
  }
...
async function handleTicketClose(interaction) {
  ...
  const isStaff = config.ticketStaffRoleId && interaction.member.roles.cache.has(config.ticketStaffRoleId);
  if (!isStaff) {
